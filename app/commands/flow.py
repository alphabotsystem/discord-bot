...
class FlowCommand(BaseCommand):
	...
	@slash_command(name="flow", description="Pull aggregated stocks orderflow.")
	async def flow(
		self,
		ctx,
		tickerId: Option(str, "Ticker id of an asset.", name="ticker", required=False, default="options"),
		...
		try:
			...
			if request.flow_available():
				outputMessage, task = await Processor.process_chart_arguments(request, arguments[1:], tickerId=arguments[0].upper(), platformQueue=["Alpha Flow"])
			...
			print(format_exc())
			if environ["PRODUCTION_MODE"]: self.logging.report_exception(user="{}: /flow {} autodelete:{}".format(ctx.author.id, " ".join(arguments), autodelete))
