...
class PriceCommand(BaseCommand):
		...
		try:
			...
			for part in parts:
				...
				if outputMessage is not None:
					embed = Embed(title=outputMessage, description="Detailed guide with examples is available on [our website](https://www.alphabotsystem.com/features/prices).", color=constants.colors["gray"])
	...
	async def price(
		...
		try:
			...
			print(format_exc())
			if environ["PRODUCTION_MODE"]: self.logging.report_exception(user=f"{ctx.author.id} {ctx.guild.id if ctx.guild is not None else -1}: /price {tickerId} type:{assetType} venue:{venue}")
			await self.unknown_error(ctx)